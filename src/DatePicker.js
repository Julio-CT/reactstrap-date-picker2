...
class DatePicker extends React.Component {
  ...
  onClickOutside(event) {
    ...
    if (this.state.focused) {
      if (
        this.overlayContainerRef &&
        this.overlayContainerRef.current &&
        !this.overlayContainerRef.current.contains(event.target)
      ) {
        const inputFocused =
          this.inputRef &&
  ...
  render() {
    ...
    return this.renderInputGroup(
      <>
        ...
        <Popover
          className={`rdp-popover ${this.state.calendarPlacement}`}
          toggle={() => this.handleHide()}
          isOpen={this.state.focused}
          container={
            this.props.calendarContainer || this.overlayContainerRef.current
          }
          target={controlId}
          placement={this.state.calendarPlacement}
          delay={200}
        >
          <PopoverHeader tag="div">{calendarHeader}</PopoverHeader>

          <PopoverBody>{calendar}</PopoverBody>
        </Popover>

        <div ref={this.overlayContainerRef} className="rdp-overlay" />
